...
class CategoryProvider extends ChangeNotifier {
  ...
  void fetchCategories() async {
    isLoaded = false;
    _categories = [];
    http.Response response = await http.get("${AppConfig.API}/category/list");
    if (response.statusCode == 200) {
      var transformer = Xml2Json();
      transformer.parse(response.body);
      List<dynamic> body = jsonDecode(transformer.toBadgerfish())['categories']['category'];
      for (dynamic i in body) {
        _categories.add(Category.fromJson(i));
      }
      for (var i = 0; i < _categories.length; i++) {
        http.Response r = await http.get(_categories[i].uriAlbums);
        if (r.statusCode == 200) {
  ...
  String getCategoryCover(Category category) {
    return category?.albums?.firstWhere((element) => element?.media?.cover != null)?.media?.cover;
  }

  String getCategoryMedium(Category category) {
    return category?.albums?.firstWhere((element) => element?.media?.medium != null)?.media?.medium;
  }
